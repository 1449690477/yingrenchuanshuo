...
/** 堆哪种词条。spd 在两把尺下都是最优键（2026-08-01 逐键实测）。 */
const AFFIX_KEY: AffixKey = 'spd';
...
function maxedRealBuild(): (EquipmentInstance | null)[] {
  return SLOT_ORDER.map((slot, i) => {
    ...
    if (!def) return null;
    const cap = QUALITY_AFFIX_COUNT[def.quality];
    const affixes: Affix[] = Array.from({ length: cap }, () => ({
      key: AFFIX_KEY,
      value: affixValueRange(AFFIX_KEY, LEVEL, 5).max,
      tier: 5 as const,
    }));
    return {
      uid: `probe-${i}`,
...
try {
  ...
  console.log(`  构造：Lv${LEVEL} ${CLASS}，每槽最强件 + 全 +${ENHANCE_MAX} + 满掷 ${EQUIPMENT_BASE_ROLL_MAX} + 全槽 T5 ${AFFIX_KEY}`);
