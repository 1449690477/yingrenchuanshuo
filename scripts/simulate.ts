 ...
 */
/*
 * 2026-08-02 清空：N4 / N5 / FRESHCP 三条随 C 类落地全部清偿，工具自身已连续
 * 打印「清偿已完成 —— 请从 GATE_CLEARANCES 删掉这条」。删掉即**恢复硬拦**：
 * 这三个门禁今后一旦被违反就是 exit 1，不再是「登记红项、只报不拦」。
 *
 * 删除前实测余量（合流态 1cd9fbb，npm run sim 退出 0）：
 *   · N4 跨职业 PvP：越界 **31 项 → 0 项**（Lv60 / Lv100 两段各 200 次模拟）
 *   · N5 职业词条极值：**全部等级段在目标带内**（此前最大偏离 26.23%，目标 ≤15%）
 *   · FRESHCP 新掉落总战力：**−6.42% ~ −1.14%**（目标 ±8%，下沿余量 1.58pp）
 * 模拟器是固定种子的确定性输出，所以余量小不会造成假红 —— 只有真实回归才会红。
 */
const GATE_CLEARANCES: readonly GateClearance[] = [
  // 目前为空 —— 没有任何门禁处在「只报不拦」状态，全部硬拦。
  // 新增条目时请一并写上负责人与燃尽期限：这张清单的价值在于它会**主动催自己清空**
  // （见 reportClearances：清偿后工具会打印「请从 GATE_CLEARANCES 删掉这条」）。
];
...
const CLASS_SPREAD_BANDS: readonly { maxLevel: number; ceiling: number; why: string }[] = [
  {
    maxLevel: 19,
    ceiling: 0.45,
    why: 'Lv10 技能池浅是设计现实（a+c 后实测 40.5% 不变，2026-08-04）；收紧条件=「低段技能解锁」停车场项落地后重测',
  },
  {
    maxLevel: 49,
    ceiling: 0.15,
    why: '中低段 a+c 后实测 ≤13.0%（2026-08-04），钉住不回退',
  },
  {
    maxLevel: 89,
    ceiling: 0.18,
    why: 'a+c 后实测 ≤15.2%（2026-08-04，c 召唤 0.40/0.56 把中段召唤过强从 24.1% 收平）；收紧条件=召唤强度设计取舍（竞技场分叉）落地后重测',
  },
  {
    maxLevel: Number.POSITIVE_INFINITY,
    ceiling: 0.19,
    why: 'a+c 后 Lv90-120 实测 ≤16.9%（2026-08-04），钉住；收紧条件同上段',
  },
];
