...
import {
  ...
  REGION_6_SET_SLOTS,
} from '../src/data/region6';
import { REGION_6_FRAGMENT_LOOT_SOURCES } from '../src/data/region6Loot';
...
const SAMPLE_SEED = 0x6262_0000;
const FRAGMENT_TARGET = REGION_6_FRAGMENT_COST * REGION_6_SET_SLOTS.length;
const FINAL_PLAYER_LEVEL = 62;
...
/**
 * 玩家不是一进新区就凭空穿齐毕业史诗：
 * 前两章沿用 R5 史诗，第三章开始换 R6 稀有，完整史诗留给毕业后的提升空间。
 */
...
function farmingStageId(day: number): string {
  if (day === 1) return 'stage_6-2_6';
  if (day === 2) return 'stage_6-4_6';
  return 'stage_6-5_6';
}
...
function representativePlayer(classId: ClassId, build: ChapterBuild) {
  ...
  for (const slot of SLOT_ORDER) {
    const definition = requireEquipment(
      `eq_${build.gearRegion}_${slot}_${build.quality}`,
    );
...
function buildForChapter(chapterId: string): ChapterBuild {
  ...
  if (!build) throw new Error(`[R6 simulation] missing chapter build: ${chapterId}`);
  return build;
}

function stageKps(stage: Stage, classId: ClassId): number {
  const monsterId = stage.waves[0]?.monsters[0]?.id;
  if (!monsterId) throw new Error(`[R6 simulation] ${stage.id} has no representative monster`);
...
function combatMetrics(): readonly CombatMetric[] {
  return CHAPTER_BUILDS.flatMap((build) => {
    ...
    if (!stage) throw new Error(`[R6 simulation] missing stage: ${stageId}`);
    const monsterId = stage.waves[0]?.monsters[0]?.id;
    if (!monsterId) throw new Error(`[R6 simulation] missing monster: ${stageId}`);
...
function buildHourBatches(): readonly HourBatch[] {
  ...
  for (let day = 1; day <= MAX_SIM_DAYS; day++) {
    ...
    const stage = STAGES[stageId];
    if (!stage) throw new Error(`[R6 simulation] missing farming stage: ${stageId}`);
    const kps = stageKps(stage, 'swordsman');
    if (!(kps > 0 && kps < 3)) {
      throw new Error(`[R6 simulation] invalid KPS at ${stageId}: ${kps}`);
    }
    for (let hour = 1; hour <= HOURS_PER_EFFECTIVE_DAY; hour++) {
      ...
      batches.push({
        ...
        sourceKillCounts: Object.fromEntries(
          REGION_6_FRAGMENT_LOOT_SOURCES.map((source) => [
            source.monsterId,
...
function firstClearEnhanceSupply(): MaterialTotals {
  ...
  for (const stage of Object.values(STAGES)) {
    if (stage.chapterId.startsWith('6-')) addDrops(totals, stage.firstClearRewards);
  }
  return totals;
}
...
function simulateEnhanceConsumption(seed: number): EnhanceConsumption {
  ...
  let attempts = 0;
  for (const slot of SLOT_ORDER) {
    const equipment = requireEquipment(`eq_r6_${slot}_rare`);
    const target = slot === 'weapon' ? 15 : 14;
    const luckByTarget: Record<string, number> = {};
    let level = 0;
    let guard = 0;
    while (level < target) {
      if (guard++ > 20_000) throw new Error(`[R6 simulation] enhance stalled: ${slot}`);
      ...
      if (result.nextLevel === null) {
        throw new Error('[R6 simulation] protected enhancement shattered');
      }
...
const LEGENDARY_IDS = SLOT_ORDER.map((slot) => `eq_r6_${slot}_legendary`);
...
function runSample(index: number): SampleResult {
  ...
  const sourceFragments = Object.fromEntries(
    REGION_6_FRAGMENT_LOOT_SOURCES.map((source) => [source.monsterId, 0]),
  );
  ...
  for (const batch of HOUR_BATCHES) {
    for (const source of REGION_6_FRAGMENT_LOOT_SOURCES) {
      ...
      for (let kill = 0; kill < count; kill++) {
        const fragmentPityKey = `${table.id}:${REGION_6_FRAGMENT_ID}`;
        ...
        const fragmentDrop =
          drops.find((drop) => drop.itemId === REGION_6_FRAGMENT_ID)?.count ?? 0;
...
function nearestRank(values: readonly number[], percentile: number): number {
  if (values.length === 0) throw new Error('[R6 simulation] percentile has no samples');
...
function finitePercentile(
  ...
  if (values.some((value) => !Number.isFinite(value))) {
    throw new Error(`[R6 simulation] ${label} has unfinished samples after ${MAX_SIM_DAYS} days`);
  }
...
function main(): void {
  ...
  const fragmentDays = {
    p50: finitePercentile(samples, (sample) => sample.completionDay, 0.5, 'shadow set'),
    p90: finitePercentile(samples, (sample) => sample.completionDay, 0.9, 'shadow set'),
    p95: finitePercentile(samples, (sample) => sample.completionDay, 0.95, 'shadow set'),
  };
  const legendaryDays = {
  ...
  const sources = REGION_6_FRAGMENT_LOOT_SOURCES.map((source) => {
    const mean = average(
  ...
  console.table([
    { target: `${FRAGMENT_TARGET} 幽影碎片`, ...fragmentDays },
    { target: 'R6 普通传说首件', ...legendaryDays },
  ]);
  ...
  writeFileSync(
    resolve(OUT_DIR, 'region6-economy.json'),
  ...
  if (fragmentDays.p50 < 6 || fragmentDays.p50 > 11) {
    violations.push(`幽影套 P50 ${round(fragmentDays.p50)} 天不在 6～11 天`);
  }
  if (fragmentDays.p95 > 14) {
    violations.push(`幽影套 P95 ${round(fragmentDays.p95)} 天超过 14 天`);
  }
  ...
  if (violations.length > 0) {
    throw new Error(`R6 经济门禁失败：\n- ${violations.join('\n- ')}`);
  }
  console.log(`✓ R6 经济门禁通过：${resolve(OUT_DIR, 'region6-economy.json')}\n`);
}
