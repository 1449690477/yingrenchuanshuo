 ...
 * 批 3（2026-08-01）按「持有口径」重标一轮：
 *   批 2-1 版按旧持有口径标定（玩家 Lv50 换传说），但 A3 修正②后
 *   typicalQualityAt 的持有口径是 epic 持有到 Lv60、legendary 到 Lv85，
 *   原表 35→65 段 0.6→1.77 的爬升让怪物攻击每 10 级 x1.8，
 *   远超玩家档内 hp/def 增速（L^1.35），EHP 真实下降，
 *   N6 品质档内成长实测掉到 1.004~1.061（初始带下限 1.12）——
 *   换装 20 级真实战力几乎不涨，是真实的成长节奏缺陷（docs/73 批 3）。
 *   重标原则：档内（品质不变段）持平或微降，补偿玩家 EHP 增速不足；
 *   品质切换点（65 epic→legendary、90 legendary→mythic、110 mythic→divine）
 *   跳升，保持切换难度陡峭感。档内段 N6 实测 1.13~1.14，留有余量。
