 ...
 *
 * ── ★ 守卫有两条，覆盖面各管一半，别只看一条 ──
 * · `cpFormulaVersion.spec.ts`（本文件下方的探针）钉的是**公式形状**：
 *   拿冻结的 Stats 喂 `combatPower`，**改公式**会红。
 * · `cpInputFingerprint.spec.ts` 钉的是**输入数据**：
 *   拿「职业+等级」算出属性再喂 `combatPower`，**改职业基础数据**会红。
 * · `affixBaselineFreeze.spec.ts` 钉的是**词条基准**（并附带阻止破坏老存档）。
 *
 * ⚠ 这里原本写的是「**这条纪律不靠人记**：指纹会红」。
 * **那句话只对改公式成立。** 2026-08-02 两套平衡方案改了词条基准，
 * 玩家战力随之改变，而当时唯一的指纹喂的是冻结 Stats、**一声不吭** ——
 * 不是有人忘了升版本，是没有任何东西会提醒他。
 * 后两条守卫就是为补这个洞加的。**再有新的 CP 输入源，要同步加守卫**，
 * 否则「不靠人记」会再一次变成一句只在纸面上成立的话。
 */
 ...
 * 3 = 五职业真实技能和怪物节奏重标后的同形公式。公式代码形状没变，
