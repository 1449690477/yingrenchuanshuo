 ...
 *
 * 主线血量基准从 60 降到 40.5 后，为了让“一场短战的总承伤”不变，
 * 主线攻击基准由 4.9 补刿到 7.26 ≈ 4.9 × 60 / 40.5。试炼和公会远征
 * 始终固定打 60 秒，没有这段 TTK 缩短，因此要反推回
 * 7.26 × 40.5 / 60 ≈ 4.9。生成时仍复用 `monsterAtk` 的等级、Boss 类型和
 * 成长公式，只把基准归一到这个固定 60 秒的等价值。
 */
export const TRIAL_MONSTER_ATK_BASE = 4.9;

/**
 * 固定 60 秒 Boss 对“典型装备基准玩家”的最大期望总承伤。
 *
 * 45% 保留狂怒周的生存压力，同时给 10% 暴击、伤害浮动与离散命中留出
 * 足够余量，确保“不设失败状态”的 60 秒成绩红线。实际门禁还会检查
 * 最险格明显掉血，防止把攻击降到完全没有威胁后“空转全绿”。
 */
export const TRIAL_REFERENCE_DAMAGE_FRACTION = 0.45;
 ...
 * Boss 血量的安全余量倍率。
 *
 * Boss 血量以「分段中位等级、主线典型品质满配玩家」的 60 秒
 * 期望输出为基准，
