...
describe('R6 强化成长原子表', () => {
  it('覆盖五章并从 R5 终章单调接续', () => {
    expect(Object.keys(REGION_6_ENHANCE_PROGRESSION)).toEqual(
      REGION_6.chapters.map((chapter) => chapter.id),
    );
    let previousAll = ENHANCE_PROGRESSION['5-5']!.recommendedAllEnhance;
    let previousMain = ENHANCE_PROGRESSION['5-5']!.recommendedMainEnhance;
    for (const chapter of REGION_6.chapters) {
      const progression = REGION_6_ENHANCE_PROGRESSION[chapter.id]!;
  ...
  it('首通强化石逐章、逐关增长', () => {
    let previousTotal = ENHANCE_PROGRESSION['5-5']!.firstClear.stoneByStage.reduce(
      (sum, count) => sum + count,
      0,
    );
    for (const chapter of REGION_6.chapters) {
      const stones = REGION_6_ENHANCE_PROGRESSION[chapter.id]!.firstClear.stoneByStage;
