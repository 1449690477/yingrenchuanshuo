 ...
 *
 * **生产同源实测**（2026-08-02，五职业、每组 12 套配对构筑 × 10 次）：
 *   d1 胜局均时 8.6~19.6s；d5 24.3~42.3s；构筑级胜局时长 P90 最大 46.5s。
 * 全部低于 67.5s 门禁，在 90 秒硬上限前保留了职业与装备方差。
 */
export const DEPTH_TARGET_MULTIPLIER: readonly number[] = [1.0, 1.5, 2.0, 2.29, 2.3];
 ...
 *
 * ── 生产同源形状（2026-08-02，五职业胜率范围）──
 *   d1 98.6~100% / d2 94.6~100% / d3 84.1~99.5%
 *   d4 43.2~85.7% / d5 4.3~29.0%（crimson 当前只开放 d1）
 *
 ...
 *
 * 2026-08-02 量具改为生产同源的技能、元素、套装战斗加成与触发器，并以同一组
 * 12 套构筑种子横比五职业。构筑级 P10/P50/P90 同时硬拦，旧具名豁免已清零。
 */
export const DEPTH_ATK_TARGET: readonly number[] = [1.0, 1.06, 1.13, 1.3, 1.9];
 ...
 * 2026-07-31 按结果侧指标先标为 `{hp: 1.25, atk: 0.55}`（配按档补偿层），
 * 批 2-3 A1 威胁因子孪生锚点表落地后，补偿层（depthTierThreatCompensation）
 * 按 docs/73 A1 契约**整体删除** —— 威胁轴漂移在源头（monsterAtk）修平，
 * 副本侧不再需要按档抬攻击的补偿；随后批 2-4 P0-2b 从「三走一试一挣」带
 * 反标定为 `{hp: 1.42, atk: 1.15}`（2026-07-31 定稿）。
 *
 * ⚠ 这两个数与两条目标曲线一起由 `npm run balance:equipment-dungeon`
 * 反标定，**不要手感微调** —— 手感微调正是造出 3.54× 跨档极差的那条路。
 */
export const DEPTH_ENCOUNTER_BASE = { hp: 1.42, atk: 1.15 } as const;
