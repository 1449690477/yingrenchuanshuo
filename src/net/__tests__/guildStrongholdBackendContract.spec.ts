...
describe('公会功勋与赛季据点服务端契约', () => {
  ...
  it('权威奖励结算失败必须让同一远征请求重试，不能静默漏发', () => {
    expect(edge).toContain('throw new Error(`公会功勋结算失败：${meritError.message}`)');
    expect(edge).toContain('throw new Error(`公会委托结算失败：${commissionError.message}`)');
    expect(edge).toContain('throw new Error(`公会据点结算失败：${strongholdError.message}`)');
    expect(edge).not.toContain("console.warn('guild merit unavailable'");
    expect(edge).not.toContain("console.warn('guild commission unavailable'");
    expect(edge).not.toContain("console.warn('guild stronghold unavailable'");
  });
});
