...
var MIN_DAMAGE_RATIO = 0.05;
var ELEM_ADVANTAGE = 1.25;
var ELEM_DISADVANTAGE = 0.85;
var ELEM_NEUTRAL = 1;
var CP_WEIGHTS = {
  atk: 2,
  def: 3,
...
function effectiveElementMultiplierFor(attackerElement, defenderElement, elementDamage) {
...
function combatBonusDamageMultiplier(defender) {
  const reductionPoints = defender.combatBonuses?.damageReduction ?? 0;
  return 1 - clamp(reductionPoints / 100, 0, 1);
}
function calcDamage(attacker, defender, skillMultiplier, rng) {
...
function damageAfterConfirmedHit(attacker, defender, atkMultiplier, element, variance, critMul) {
...
function applyDamageAndLifesteal(attacker, defender, rolledDamage, lethalTriggers = [], lethalUses = /* @__PURE__ */ new Map()) {
...
function resolveLethalRecovery(defender, triggers, uses) {
...
function simulateDuel(attacker, defender, rng) {
...
function duelSeed(attackerId, defenderId, dayKey, attemptIndex) {
...
var equipmentInstanceSchema = z.object({
  ...
  if (pending.operation === "inscribe" && !isProfessionAffixSlot(definition.quality, instance.affixes.length, pending.affixIndex)) {
    ctx.addIssue({
      code: "custom",
      path: ["pendingAffixChange", "affixIndex"],
...
var milestoneRecordSchema = z.object({
